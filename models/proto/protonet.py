...
#         y_true = target_inds.reshape(-1).cpu().numpy()
#         y_pred = y_hat_supervised.cpu().numpy()
#         precision, recall, f1 = self.compute_metrics(y_true, y_pred)
...
#     # Plotting the loss vs number of iterations
#     plt.figure()
#     plt.plot(range(max_iter), all_train_losses, label='Training Loss')
#     plt.xlabel('Number of Iterations')
#     plt.ylabel('Loss')
#     plt.title('Loss vs Number of Iterations')
#     plt.legend()
#     plt.savefig(os.path.join(output_path, 'loss_vs_iterations.png'))
#     plt.close()
...
class ProtoNet(nn.Module):
    ...
    def loss(self, sample, supervised_loss_share: float = 0):
        ...
        if has_augmentations:
            # Unsupervised loss
            unsupervised_target_inds = torch.arange(0, n_augmentations_samples).to(device).long()
            unsupervised_loss = CrossEntropyLoss()(-unsupervised_dists, unsupervised_target_inds)
            _, y_hat_unsupervised = (-unsupervised_dists).max(1)
            acc_val_unsupervised = torch.eq(y_hat_unsupervised, unsupervised_target_inds.reshape(-1)).float().mean()
...
def run_proto(
            ...
            if (step + 1) % evaluate_every == 0:
                for path, writer, set_type, set_data_loader in zip(
                    ...
                    if path:
                        ...
                        logger.info(f"{set_type} | " + " | ".join([f"{key}:{np.mean(value):.4f}" for key, value in set_results.items()]))
                        if set_type == "test":
                            test_accuracies.append(set_results["acc"])
                        if set_type == "valid":
                            if set_results["acc"] > best_valid_acc:
                                best_valid_acc = set_results["acc"]
                                n_eval_since_last_best = 0
    ...
    # Plotting the test accuracy vs number of iterations
    plt.figure()
    plt.plot(range(0, max_iter, evaluate_every), test_accuracies, label='Test Accuracy')
    plt.xlabel('Number of Iterations')
    plt.ylabel('Accuracy')
    plt.title('Test Accuracy vs Number of Iterations')
    plt.legend()
    plt.savefig(os.path.join(output_path, 'test_accuracy_vs_iterations.png'))
    plt.close()
